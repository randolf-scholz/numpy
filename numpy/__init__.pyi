...
_date = TypeVar("_date", bound=dt.date)
_datetime = TypeVar("_datetime", bound=dt.datetime)
...
class datetime64(_RealMixin, generic[_DT64ItemT_co], Generic[_DT64ItemT_co]):
    ...
    # NOTE: overload order must be very precise!
    #   Start with most concrete types, and end with most generic types
    ...
    @overload  # timestamp[int] - duration[int | date] = duration[int]
    def __sub__(self: datetime64[int], x: datetime64[int | dt.date], /) -> timedelta64[int]: ...
    @overload  # timestamp[int] - duration[int | date] = timestamp[int]
    def __sub__(self: datetime64[int], x: timedelta64[int | dt.timedelta], /) -> datetime64[int]: ...
    @overload  # timestamp[int | dt.date] - timestamp[int] = duration[int]
    def __sub__(self: datetime64[int | dt.date], x: datetime64[int], /) -> timedelta64[int]: ...
    @overload  # timestamp[int | dt.date] - duration[int] = timestamp[int]
    def __sub__(self: datetime64[int | dt.date], x: timedelta64[int], /) -> datetime64[int]: ...

    # Priority 4b:
    # @overload  # timestamp[int] - duration[unknown] = duration[int | None]
    # def __sub__(self: datetime64[int | dt.date], x: datetime64, /) -> timedelta64[int | None]: ...
    # @overload  # timestamp[int] - duration[unknown] = timestamp[int | None]
    # def __sub__(self: datetime64[int | dt.date], x: timedelta64, /) -> datetime64[int | None]: ...
    # @overload  # timestamp[unknown] - timestamp[int] = duration[int]
    # def __sub__(self: datetime64, x: datetime64[int | dt.date], /) -> timedelta64[int | None]: ...
    # @overload  # timestamp[unknown] - duration[int] = timestamp[int]
    # def __sub__(self: datetime64, x: timedelta64[int | dt.date], /) -> datetime64[int | None]: ...
    ...
    @overload  # timestamp[unknown] - duration[time] = timestamp[date]
    def __sub__(self, x: timedelta64[dt.timedelta], /) -> datetime64[dt.datetime]: ...

    # Priority 5: fallback
    @overload  # timestamp[unknown] - timestamp[unknown] = duration[unknown]
    def __sub__(self, x: datetime64, /) -> timedelta64: ...
    @overload  # timestamp[unknown] - duration[unknown] = timestamp[unknown]
    def __sub__(self, x: timedelta64, /) -> datetime64: ...
